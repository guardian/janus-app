...
class WriterTest extends FreeSpec with Matchers {
  ...
  "allPermissions" - {
    ...
    "includes default access permissions" in {
      val permission = Permission(account1, "perm1", "Test permission", "", false)
      val janusData = JanusData(
        Set.empty,
    ...
    "includes access permissions from mulitple users" in {
      val permission1 = Permission(account1, "perm1", "Test permission 1", "", false)
      val permission2 = Permission(account1, "perm2", "Test permission 2", "", false)
      val janusData = JanusData(
        Set.empty,
        access = ACL(Map(
    ...
    "includes admin permissions" in {
      ...
      val permission2 = Permission(account1, "perm2", "Test permission 2", "", false)
      val janusData = JanusData(
        Set.empty, ACL(Map.empty, Set.empty),
        admin = ACL(Map(
    ...
    "includes permissions from all sources" in {
      val permission1 = Permission(account1, "perm1", "Test permission 1", "", false)
      val permission2 = Permission(account1, "perm2", "Test permission 2", "", false)
      val permission3 = Permission(account1, "perm3", "Test permission 3", "", false)
      val permission4 = Permission(account1, "perm4", "Test permission 4", "", false)
      val permission5 = Permission(account1, "perm5", "Test permission 5", "", false)
      val janusData = JanusData(
        Set.empty,
        access = ACL(Map(
          "user1" -> Set(permission1),
