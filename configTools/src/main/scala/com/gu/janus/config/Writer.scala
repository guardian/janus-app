...
object Writer {
  def toConfig(janusData: JanusData): String = {
    ...
    stripWhitespace(templates.txt.janusData(janusData, uniquePermissions).toString)
  }
  ...
  /**
    * Twirl is designed for HTML, not plain text. As a result it's tricky to control
    * how whitespace (particularly newlines) get added to the file.
    *
