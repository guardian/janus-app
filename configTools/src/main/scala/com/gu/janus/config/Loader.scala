...
object Loader {
  ...
  private[config] def loadPermissions(config: Config, accounts: Set[AwsAccount]): Either[String, Set[Permission]] = {
    for {
      ...
      permissions <- configuredPermissions.permissions.traverse { configuredPermission =>
        for {
  ...
  private[config] def loadAccess(config: Config, permissions: Set[Permission]): Either[String, ACL] = {
    for {
      ...
      defaultAccess <- configuredAccess.defaultPermissions.traverse { configuredAclEntry =>
      ...
      acl <- configuredAccess.acl.toList.traverse { case (username, configuredAclEntries) =>
        for {
          userPermissions <- configuredAclEntries.traverse { configuredAclEntry =>
            permissions.find(p => configuredAclEntry.account == p.account.authConfigKey && configuredAclEntry.label == p.label)
  ...
  private[config] def loadSupport(config: Config, permissions: Set[Permission]): Either[String, SupportACL] = {
    for {
      ...
      supportAccess <- configuredSupport.supportAccess.traverse { configuredAclEntry =>
        permissions.find(p => configuredAclEntry.account == p.account.authConfigKey && configuredAclEntry.label == p.label)
