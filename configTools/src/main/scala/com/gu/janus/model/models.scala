...
object SupportACL {
  /**
    * Normalise the data on creation so it is impossible to have a bad representation.
    */
  def create(rota: Map[DateTime, (String, String)], supportAccess: Set[Permission], supportPeriod: Period): SupportACL = {
    val rotaWithNormalisedTimezones = rota.map { case (dt, a) => dt.withZone(DateTimeZone.UTC) -> a }
    new SupportACL(rotaWithNormalisedTimezones, supportAccess, supportPeriod.toStandardSeconds)
  }
}
...
object Permission {
  def apply(account: AwsAccount, label: String, description: String, policy: Policy, shortTerm: Boolean = false): Permission = {
