...
class AppComponents(context: ApplicationLoader.Context)
  ...
  override def httpFilters: Seq[EssentialFilter] = super.httpFilters :+ new HstsFilter
  ...
  Config.validateAccountConfig(janusData, configuration) match {
    ...
    case ConfigWarn(accounts) =>
      logger.warn(s"Account(s) present in config that are not present in the JanusData: ${accounts.mkString(", ")}")
    case ConfigError(accounts) =>
      throw new RuntimeException(s"One or more accounts are missing from config: ${accounts.mkString(", ")}")
    case ConfigSuccess =>
  }
  ...
  override def router: Router = new Routes(
    httpErrorHandler,
    new Janus(janusData, controllerComponents, authAction, host, Clients.stsClient, configuration)(dynamodDB, assetsFinder),
    new Audit(janusData, controllerComponents, authAction)(dynamodDB, assetsFinder),
    new RevokePermissions(janusData, controllerComponents, authAction, Clients.stsClient, configuration)(assetsFinder),
    new AuthController(janusData, controllerComponents, googleAuthConfig, googleGroupChecker, requiredGoogleGroups)(wsClient, executionContext, assetsFinder),
