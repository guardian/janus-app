...
object AuditTrailDB {
  ...
  def getAccountLogs(table: Table, account: String, startDate: DateTime, endDate: DateTime)(implicit dynamoDB: DynamoDB): Seq[Either[String, AuditLog]] = {
    dynamoDB.query(
      table,
      keyConditions = Seq(
        "j_account" -> DynamoDBCondition.eq(account),
        "j_timestamp" -> DynamoDBCondition.between(startDate.getMillis, endDate.getMillis)
      ),
      scanIndexForward = false
    ).map(_.attributes) map auditLogFromAttrs map logDbResultErrs map errorStrings
  }

  def getUserLogs(table: Table, username: String, startDate: DateTime, endDate: DateTime)(implicit dynamoDB: DynamoDB): Seq[Either[String, AuditLog]] = {
