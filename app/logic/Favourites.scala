...
object Favourites extends Logging {
  def fromCookie(cookie: Cookie): List[String] = {
    try {
      Json.parse(Base64.getDecoder.decode(cookie.value)).validate[List[String]].fold(
      { a =>
      ...
      case e @ (_: JsonMappingException | _: JsonParseException | _: IllegalArgumentException) =>
        logger.warn(s"Failed to parse favourites cookie value: ${cookie.value}", e)
        Nil
    }
  }
  ...
  def toCookie(favourites: List[String]): Cookie = {
    val value = Base64.getEncoder.encodeToString(Json.stringify(Json.toJson(favourites)).getBytes("UTF-8"))
